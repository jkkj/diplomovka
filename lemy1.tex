...
\begin{lem}
Agent na ceste do vrchola v grafe musí ísť po už existujúcich hranách.
\end{lem}
...
\begin{lem}
Ak agent odíde z vrcholu v ktorom je počet súčet incidentných hrán a
otvorených portov nižší, ako je počet hrán s ktorými má byť vrchol
incidentný v konštruovanom grafe, tak sa agent doň musí neskôr vrátiť.
\end{lem}

\begin{proof}
Neexistuje postupnosť operácií pomocou ktorej sa dá vytvoriť hrana do
existujúceho vrcholubez jeho návštevy, ak v ňom nie je otvorený port pre
túto hranu.
\end{proof}

\begin{lem}
Hrany vytvorené operáciou "nová hrana" tovria kostru grafu.
\end{lem}
\begin{proof}
...
\end{proof}
\begin{lem}
Hrany, ktoré prejdem od otvorenia prvého portu hrany po otvorenie druhého,
tvoria v grafe s novovzniknutou hranou uzavretý sled.
\end{lem}
\begin{proof}
Je to dané operáciami, ktoré môže agent vykonať.
\end{proof}
\begin{pozn}
Z tohto sledu viem vybrať kružnicu.
\end{pozn}
\begin{lem}
Minimálny počet hrán, ktoré potrebujem prejsť pri konštrukcii hrany pomocou
portov je rovný alebo väčší ako dĺžka najmenšej kružnice, na ktorej sa táto
hrana nachádza.
\end{lem}
\begin{proof}
Nájdem menší takýto sled ako je najkratšia kružnica. Z neho viem vybrať
kružnicu, čo je spor s minimálnosťou najkratšej kružnice.
\end{proof}
\begin{lem}
Ak mám k dispozícii dostatočný počet portov, problém efektívnej konštrukcie grafu sa
redukuje na hľadanie najkratšieho sledu v ktorom sú všetky vrcholy. Pri
hamiltonovskom grafe ide o hamiltonovskú kružnicu.
\end{lem}
\begin{proof}
Agent prechádza vrcholmi, pričom mu sačí jedna návšteva každého vrcholu v ktorom
otvorí potrebný počet portov príslušných značiek.
\end{proof}
\begin{lem}
Vrchol v musím navštíviť minimálne $\frac{deg(v)}{\#značka * 2} $ krát.
\end{lem}
\begin{proof}
Pri každej návšteve viem otovrením portov so všetkými značkami vytvoriť nové
hrany do vrcholov, kde sú otovrené príslušné porty a následne otvoriť toľko
portov koľko je značiek, každý s inou značkou.
\end{proof}

\subsection{Jedna značka pre porty}
\begin{lem}
Dolný odhad počtu krokov je minimum zo súčtov minimálnych kružníc pre všetky
nekostrové hrany cez všetky kostry.
\end{lem}
\begin{proof}
Menej ako najkratšia kružnica to agent urobiť nevie. Port pre ďalšiu hranu môže
otvoriť port, až keď už vznikla predošlá. Od tohto momentu môže otvoriť port
pre ďalšiu hranu a prejsť minimálne najkratšiu kružnocu na ktorej táto hrana
leží a na konci otvoriť druhý port tejto hrany.
\end{proof}
\begin{lem}
TODO zovšeobecniť prečo sa pri cyklickej mriežke tak našlapem a pri kocke
nie ... a dokázať :)
\end{lem}
\begin{proof}
\end{proof}
...
\begin{lem}
Po použití operácie NH, ktorou vznikne hrana h, sa nedostanem do už existujúceho vrchola bez použitia
operácie OP, pri ktorej vznikne hrana alebo vracania sa po hrane h.
\end{lem}
\begin{proof}
...
Vtedy, ak agent prechádza vrcholom s ktorým bude incidentná nasledujúca
hrana, v ňom otvorí port a následne sa doň už nemusí vracať ak existuje
kratšia cesta.

\section{Mriežka}
Najmenší cyklus má dĺžku štyri. Teda medzi otvorením dvoch portov pre danú
hranu vykoná agent najmenej tri pohyby po hranách.
...
\end{proof}
\begin{lem}
Pri rotačnej mriežke treba ''odšľapať'' oba ''rotačné cykly'' ... TODO
\end{lem}
\begin{proof}
TODO ...
\end{proof}
